...
class OdomNew
{
  public:
    ...
    void compute_and_publish_odom()
    {
      ...
      //////// TODO //////
      vx  =  (this->radius*(right_vel+left_vel))/2;
      vth = (this->radius*(right_vel-left_vel))/(this->wheel_distance);
      this->x  += (vx*dt)*cos(this->th+(dt*vth)/2); 
      this->y  += (vx*dt)*sin(this->th+(dt*vth)/2);
      this->th += dt*vth;
      this->th = (this->th<-2*M_PI)?0:((this->th>2*M_PI)?0:this->th);
      ROS_INFO("position(x=%f,y=%f,th=%f)", this->x,this->y,this->th);
